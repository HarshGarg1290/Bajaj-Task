...
const DoctorListing = () => {
	...
	const [consultationMode, setConsultationMode] = useState(""); // New state for consultation mode
	...
	const clearAllFilters = () => {
		...
		setSortBy("price");
		setSearchQuery("");
		setConsultationMode(""); // Clear consultation mode filter
	};
	...
	const filteredAndSortedDoctors = useMemo(() => {
		...
		if (consultationMode === "video") {
			result = result.filter((doctor) => doctor.video_consult === true);
		} else if (consultationMode === "clinic") {
			result = result.filter((doctor) => doctor.in_clinic === true);
		}
		...
		if (sortBy === "price") {
			...
			result.sort(
				(a, b) =>
					parseExperienceYears(b.experience) -
					parseExperienceYears(a.experience)
			);
		}

		return result;
	}, [doctors, searchQuery, selectedSpecialities, sortBy, consultationMode]);
				...
				<div className="max-w-6xl mx-auto">
					<div className="relative">
						...
						<Input
							...
							className="w-full pl-10 pr-4 py-2 rounded-lg text-gray-200"
							value={searchQuery}
							...
							>
								...
								<div className="flex items-center space-x-2">
									...
									<label htmlFor="experience">
										Experience- <br></br> Most Experience first
									</label>
								</div>
							</RadioGroup>
						</div>

						{/* New Consultation Mode Filter */}
						<div className="mb-6">
							<h3 className="text-lg font-medium mb-4">Mode of Consultation</h3>
							<RadioGroup
								value={consultationMode}
								onValueChange={(value) => setConsultationMode(value)}
							>
								<div className="flex items-center space-x-2 mb-2">
								...
								<div className="flex items-center space-x-2 mb-2">
									<RadioGroupItem value="clinic" id="in-clinic" />
									<label htmlFor="in-clinic">In Clinic</label>
								</div>
								<div className="flex items-center space-x-2">
									<RadioGroupItem value="" id="any-mode" />
									<label htmlFor="any-mode">Any Mode</label>
								</div>
							</RadioGroup>
						</div>
									...
									availableSpecialities.map((speciality) => (
										...
										>
											<Checkbox
												...
												onCheckedChange={() =>
													handleSpecialityChange(speciality)
												}
											/>
											<label
												htmlFor={speciality.toLowerCase().replace(/\s+/g, "-")}
											>
												{speciality}
											</label>
										</div>
									...
									<div className="flex items-start justify-between">
										...
										<div className="text-right">
											<p className="text-lg font-medium">
												{doctor.fees || "Fee not available"}
											</p>
											{/* Display consultation modes available */}
											<div className="flex flex-col gap-1 mt-1 text-sm">
												{doctor.video_consult && (
													<span className="text-blue-600">✓ Video Consult</span>
												)}
												{doctor.in_clinic && (
													<span className="text-green-600">✓ In Clinic</span>
												)}
											</div>
